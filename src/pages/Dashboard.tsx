...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('profiles').select('balance, savings_balance').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('transfers').select('*').eq('user_id', user.id).order('created_at', {
        ascending: false
      }).limit(5);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('internal_transfers').select('*').or(`sender_id.eq.${user.id},recipient_id.eq.${user.id}`).order('created_at', {
        ascending: false
      }).limit(5);
      return data || [];
    },
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,tether,solana,ripple,binancecoin&order=market_cap_desc');
      if (!res.ok) return [];
      return res.json();
    },
  ...
  return <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border sticky top-0 z-50">
        ...
        {/* Mobile Menu */}
        {menuOpen && <div className="md:hidden border-t border-border p-4 space-y-2 bg-card">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-gradient-to-br from-primary to-primary/80 rounded-2xl p-6 text-white">
            ...
            <div className="text-3xl font-bold mb-4">
              {showBalance ? `$${bankBalance.toLocaleString('en-US', {
          ...
          <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-2xl p-6 text-white">
            ...
            <div className="text-3xl font-bold mb-4">
              {showBalance ? `$${savingsBalance.toLocaleString('en-US', {
              minimumFractionDigits: 2
            })}` : '••••••••'}
            </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white">
            ...
            <div className="text-3xl font-bold mb-4">
              {showBalance ? `$${totalCryptoBalance.toLocaleString('en-US', {
              minimumFractionDigits: 2
            })}` : '••••••••'}
            </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          {transfersLoading ? <div className="text-center py-8 text-muted-foreground">Loading transactions...</div> : transfers.length === 0 ? <div className="text-center py-8">
              ...
              {transfers.map(transaction => <div key={transaction.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                  <div className="flex items-center gap-4">
