...
import { CreditCard, Wallet, TrendingUp, Send, Download, ArrowUpRight, Bell, Settings, LogOut, Menu, X, Eye, EyeOff } from 'lucide-react';
export default function Dashboard() {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border sticky top-0 z-50">
        ...
        {/* Mobile Menu */}
        {menuOpen && <div className="md:hidden border-t border-border p-4 space-y-2">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="space-y-4">
            {transactions.map(transaction => <div key={transaction.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                <div className="flex items-center gap-4">
                  <div className={`rounded-lg p-2 ${transaction.type === 'received' ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                    {transaction.type === 'received' ? <Download className={`h-4 w-4 text-green-500`} /> : <Send className={`h-4 w-4 text-red-500`} />}
                  </div>
                  <div>
